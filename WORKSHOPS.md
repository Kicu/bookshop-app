...
## Ex. #1: add redux to the application
 - install redux and create a file `configureStore` inside `redux` folder
 - write simple `helloWorld` reducer,
 ...
 so use the `combineReducers` function and pass your test/hello reducer to it, helpful links:
   - https://redux.js.org/basics/store#source-code
   - https://redux.js.org/api-reference/combinereducers#reducersindex.js
 - from the file export a function for building the store rather than the created store (why?), the function should simply return the created store when called
 ...
 - add the devtools extension:
   - we will be using the most known: https://github.com/zalmoxisus/redux-devtools-extension
   - we will be using browser plugin + code written inside our `configureStore` (no npm module!!!)
   - start with the "Basic store" setup (https://github.com/zalmoxisus/redux-devtools-extension#11-basic-store)
...
### 2.1 create the module and reducer
 ...
 Whenever we will add a new functionality/piece of data or schema we will try to scope it into something called a **module**.
 ...
 - create the reducer: put it in the file `reducer.js` and re-import and export it through `featured/index.js` file (don't forget about default state!)
 - you will (probably ;)) need some action-types; add a file called `actionTypes.js` and create 3 action types representing pending, success and failure states
 (suggested standard: `FETCH_X`, `FETCH_X_SUCCESS`, `FETCH_X_ERROR`)
 - add the reducer to global store by passing it to argument that goes to `combineReducers` named simply: `featured`
...
### 2.2 async + data
 ...
 Now write an action creator (thunk) responsible for getting the data:
  - create `featured/actions.js` file and inside create a `fetchFeatured`
  - first dispatch the pending-state action
...
### 2.3 binding redux store to react
  ...
  - add `Provider` to your app (https://redux.js.org/basics/usage-with-react#index-js)
  
  React:
   - Let's start with a component called `FeaturedList`
   - use connect to give the component access to the store:
     - help: https://redux.js.org/basics/usage-with-react#containers-filterlink-js
     - write `mapStateToProps`, it should pass `featured` state to the component
     - write `mapDispatchToProps`, it should pass `fetchFeatured` with dispatch to the component
     - you can use the shortcut version using object instead of function: `mapDispatchToProps = { fetchFeatured }`
   - remember to actually call the fetch action somewhere to get the data - what's the proper place?
   - the `FeaturedList` render method should return a simple wrapping div and then `ul > li` with the featured books
...
### 3.1 Categories
 ...
 For starters render the component inside the main App.
 ...
 *Note:*
 ...
 Things to do:
  - write new module similarly to `featured`
  - new actionTypes + actions (thunks) for fetching books by category
  - use `getBooksByCategory` function from the fakeApi file
  - write a new reducer, let's call it similar to your component for now; can be simply `categories` or `booksByCategory`
  - don't forget to add the reducer to the store
  - create the actual `<CategoryList/>` component and add connect (with mapState + mapDispatch)
...
### 3.3 Routing
 ...
 Docs: https://reacttraining.com/react-router/web/guides/quick-start/installation
 
 Now let's create the routing:
 - wrap the app with `Router`
 - add a route: `/category/:category` and add our component as a handler
 - modify the `mapState` function to be able to grab the category name from the rr params (we conveniently called it `category` ;) )
 - you should be able to type something similar to `/category/fantasy` and see the book list
 ...
 - make sure that new books are fetched every time the category name changes (how?)
...
## Ex. #4: 1st refactor (views - React)
 ...
 Now let us use this knowledge to make rendering our book lists much better: 
 - let's create `<BookList />`; make it a "dumb" component that takes a books array as property and displays it
 - I suggest renaming classes in this component and styling to `BookList` to make stuff still look pretty
 ...
 - refactor both components to use `BookList` component; make them `containers` that fetch data and provide the data to BookList
...
## Ex. #5: Book Search
 ...
 ### 5.1 UI part:
 - create a new `<Search/>` component, we won't be treating it as a "screen" (but it should be "smart") since we don't want to make it a route handler
 - it will be responsible for the whole searching functionality:
   - input for typing
   - displaying search results
 - searching for books should be triggered on change when user is typing the phrase
...
### 6.1 selectors
  Selectors are are functions that describe how to get specific data from the store.
  They usually get `state` as a first argument and may accept more arguments when needed.
  
  ```
      You don't need reselect to write selectors
  ```
  ...
  As a warm up:
   ...
   - selectors can go into their own file, but also `reducer.js` is a good place of storing them as named exports
   - remember about usage of `index` file for the modules
 ...
 #### 2. rewrite your featured reducer to save only ids of books
  ...
  - rewrite your `getFeaturedBooks` selector to properly return full book data using the `books` module we created
 ...
 a implementation detail of that module - something that can change freely if developer decides
 
 Let's fix it:
 - in books module write a selector called `getBooksByIds` taking an array of ids and returning array of books
 - reuse it in featured/categories
...
## Ex. #7: Basket
 What kind of store could run without a basket? We will now add one and the ability to add books to the basket.
